...
const dr = dumber({
  ...
  onManifest: isTest ? undefined : function (filenameMap) {
    ...
    fs.writeFileSync(path.join(__dirname, ...siteRoot.split('/'), 'index.html'), indexHtml);
  }
});
...
function build() {
  ...
  return merge2(
    ...
    buildJs(['src/app/**/*.ts', 'src/shared/**.ts']),
    buildHtml('src/app/**/*.html'),
    buildCss('src/app/**/*.css')
  )
    ...
    .pipe(gulp.dest(scriptsPath, { sourcemaps: isProduction ? false : '.' }));
}

function clean() {
  return del(scriptsPath);
}
...
function watch() {
  gulp.watch(['src/app/**/*','src/shared/**/*'], gulp.series(build, reload));
}
